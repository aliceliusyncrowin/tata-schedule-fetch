...
@app.timer_trigger(schedule="0 */15 * * * *", arg_name="myTimer", run_on_startup=False,
              use_monitor=False) 
def get_tata_scheduler(myTimer: func.TimerRequest) -> None:
    if myTimer.past_due:
    ...
    try:
        ...
        data = call_tata_schedule.get_utility_data(token, plant_name="Pavagada", date="2026-06-03", schd_rev_no=-1)
        logging.info(f"Utility data obtained: {data}")
